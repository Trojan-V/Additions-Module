...
// TODO: check all actions if they are registered for both CHATFILTER and MAIN ScriptContext
public class BetterFor extends ScriptActionDo {
    ...
    public boolean executeStackPush(IScriptActionProvider provider, IMacro macro, IMacroAction instance, String rawParams, String[] params) {
        ...
        try {
            ...
            Game.addChatMessage("Couldn't get constructor. (Error message came from: Additions Module -> BetterFor )");
            e.printStackTrace();
            return false;
        }
        ...
        if (state == null) {
            ...
            try {
                ...
                Game.addChatMessage("constructor#newInstance() invocation failed for some reason. (" + e.getMessage() + "). (Error message came from: Additions Module -> BetterFor )");
                e.printStackTrace();
            }
        ...
        if(!(isActiveO instanceof Boolean)) {
            Game.addChatMessage("Something went wrong in the for loop (isActive is not a boolean). (Error message came from: Additions Module -> BetterFor )");
            return false;
        }
        ...
        if(!(getVariableName instanceof String)) {
            Game.addChatMessage("Something went wrong in the for loop (getVariableName is not a string). (Error message came from: Additions Module -> BetterFor )");
            System.out.println("GetVariable: " + getVariableName.getClass().getName());
            return false;
        }

        if(!(getCounter instanceof Integer)) {
            Game.addChatMessage("Something went wrong in the for loop (getCounter is not an integer). (Error message came from: Additions Module -> BetterFor )");
